...
.controller('sortController', function(filtersFactory){
	...
	// css заменяет
	/* 
	this.buttonIds = ["menuAuthor", "menuPrice", "menuName", "menuCategories"];

	this.closeAllSort = function() {
		for (var i = 0; i < this.buttonIds.length; i++) {
			document.getElementById(this.buttonIds[i]).classList.remove("show");
		}
	}

	this.isHidden = function(buttonId) {
		return !document.getElementById(buttonId).classList.contains("show")
	}

	this.toggleElement = function(buttonId) {
		var show = this.isHidden(buttonId);
		this.closeAllSort();
		if(show)
	...
	this.openAuthor = function() {
		this.toggleElement("menuAuthor");
	}

	this.openPrice = function() {
		this.toggleElement("menuPrice");
	}

	this.openName = function() {
		this.toggleElement("menuName");
	}

	this.openCategories = function() {
		this.toggleElement("menuCategories");
	}
	*/
});